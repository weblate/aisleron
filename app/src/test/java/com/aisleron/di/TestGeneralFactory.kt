/*
 * Copyright (C) 2025-2026 aisleron.com
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program.  If not, see <https://www.gnu.org/licenses/>.
 */

package com.aisleron.di

import com.aisleron.data.sync.SyncSchedulerTestImpl
import com.aisleron.domain.TransactionRunner
import com.aisleron.domain.sync.SyncScheduler
import com.aisleron.testdata.data.TransactionRunnerTestImpl

class TestGeneralFactory {
    val syncScheduler: SyncScheduler by lazy {
        SyncSchedulerTestImpl()
    }

    val transactionRunner: TransactionRunner by lazy {
        TransactionRunnerTestImpl()
    }

    inline fun <reified T> get(): T {
        return when (T::class) {
            SyncScheduler::class -> syncScheduler as T
            TransactionRunner::class -> transactionRunner as T

            else -> throw Exception("Unknown class ${T::class}")
        }
    }
}